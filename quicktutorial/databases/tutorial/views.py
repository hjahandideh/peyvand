...
from .models import Session , Nameh , User,Paygham,payam,Image,etela,Delet_Nameh,user_group_table
...
class WikiViews ( object ):
  ...
  def nameh_edit(self):
       ...
       payamr=Session.query(payam).filter_by(recive=self.logged_in,state="0").count()
       ...
       return dict( nav=n,payamr=payamr,log=log,mag=mag,newre=newre,id=page.id,nnameh=page.nnameh , mnameh=page.mnameh , chnameh=page.chnameh , manameh=page.manameh,recive=page.recive,ersal=page.ersal,tarikher=page.tarikher,mohlat=page.mohlat,jahat=page.jahat,peyvast=page.peyvast,vaseiyat=page.vaseiyat )
  @view_config ( route_name='delet_nameh' , renderer='template/delet_nameh.pt',permission='entry' )
  def nameh_delet(self):
       n=Session.query(Nameh).filter_by(recive=self.logged_in, vaseiyat="F").count()
       page1=Session.query(Delet_Nameh).filter_by(ersal=self.logged_in)
       log=Session.query(User).filter_by(username=self.logged_in)
       payamr=Session.query(payam).filter_by(recive=self.logged_in,state="0").count()
       mag=Session.query(Image).filter_by(name=self.logged_in)
       newre=Session.query(User.username)
       ...
       return dict( nav=n,payamr=payamr,log=log,mag=mag,newre=newre,page1=page1)
  ...
  def nameh_view(self):
       ...
       if 'form_submitted' in request.params:
            ...
            Session.add (Nameh ( nnameh=nnameh , mnameh=mnameh , chnameh=chnameh , manameh=manameh,recive=recive,ersal=ersal,tarikher=tarikher,mohlat=mohlat,emza=emza,jahat=jahat,peyvast=peyvast,vaseiyat=vaseiyat ) )

            return HTTPFound ( 'ersali' )

       return dict (payamr=payamr,nav=n,mag=mag,newre=newre,log=log)
  ...
  def view_payam(self):
        ...
        pag.state="1"
        return dict(state=pag.state,page=page,log=log,mag=mag,payamr=payamr,nav=n,nam=p)
  @view_config(route_name='delet' )
  def delnameh(self):
       id = int(self.request.matchdict['id'])
       page=Session.query(Nameh).filter_by(id=id).one()
       Session.add(Delet_Nameh(nnameh=page.nnameh , mnameh=page.mnameh , chnameh=page.chnameh , manameh=page.manameh,ersal=page.ersal,recive=page.recive,tarikher=page.tarikher,mohlat=page.mohlat,emza=page.emza,jahat=page.jahat,peyvast=page.peyvast,vaseiyat=page.vaseiyat ))
       Session.delete(page)
       HTTPFound('delet_nameh')
  ...
  def viewu_payam(self):
        ...
        return dict(page=page,log=log,mag=mag,payamr=payamr,nav=n,nam=p)
  ...
  def view_nameh(self):
        ...
        return dict(page=page,payamr=payamr,log=log,mag=mag,nav=n,vaseiyat=pag.vaseiyat)
  ...
  def recive_payam(self):
            ...
            return dict (log=log,dpayam=dpayam,mag=mag,nav=n,payamr=payamr)
  @view_config ( route_name='view_etla' , renderer='template/view_etla.pt',permission='entry'  )
  def veiw_etla(self):
        id = int(self.request.matchdict['id'])
        pag = Session.query(etela).filter_by(id=id).one()
        log=Session.query(User).filter_by(username=self.logged_in)
        mag=Session.query(Image).filter_by(name=self.logged_in)
        n=Session.query(Nameh).filter_by(recive=self.logged_in, vaseiyat="F").count()
        payamr=Session.query(payam).filter_by(recive=self.logged_in,state="0").count()
        page=Session.query(Paygham).all()
        return dict (log=log,mag=mag,page=page,nav=n,payamr=payamr,pag=pag)

  # admin

  @view_config ( route_name='new_etla' , renderer='template/m_new_etla.pt',permission='topsecret'  )
  def new_etla(self):
        ...
        if 'frm-etla' in request.params:
            ...
            Session.add(etela(title=title,etla=etla,created_date=created_date))
            return HTTPFound('view_etla')
        log=Session.query(User).filter_by(username=self.logged_in)
        mag=Session.query(Image).filter_by(name=self.logged_in)
        n=Session.query(Nameh).filter_by(recive=self.logged_in, vaseiyat="F").count()
        payamr=Session.query(payam).filter_by(recive=self.logged_in,state="0").count()
  ...
  @view_config ( route_name='mview_etla' , renderer='template/m_view_etla.pt',permission='topsecret'  )
  def mveiw_etla(self):
        id = int(self.request.matchdict['id'])
        pag = Session.query(etela).filter_by(id=id).one()
        log=Session.query(User).filter_by(username=self.logged_in)
  ...
  def mnameh_edit(self):
       ...
       if 'form_submitted' in request.params:
            ...
            return HTTPFound ( '/mpishnevis' )

       return dict( nav=n,log=log,mag=mag,newre=newre,payamr=payamr,id=page.id,nnameh=page.nnameh , mnameh=page.mnameh , chnameh=page.chnameh , manameh=page.manameh,recive=page.recive,ersal=page.ersal,tarikher=page.tarikher,mohlat=page.mohlat,jahat=page.jahat,peyvast=page.peyvast,vaseiyat=page.vaseiyat )
  ...
  def mnameh_view(self):
       ...
       if 'form_submitted' in request.params:
            ...
            return HTTPFound ( 'mersali' )

       return dict (nav=n,mag=mag,newre=newre,log=log,payamr=payamr)
  ...
  def mimg_view(self):
    ...
    if "frmimg" in request.params:
      ...
      return HTTPFound('/modirusername')
    return dict(mag=mag,log=log,nav=n,payamr=payamr)
  ...
  def user_add(self):
       ...
       return dict(log=log,mag=mag,nav=n,payamr=payamr)
  ...
  def mpishnevis(self):
            ...
            usr=Session.query(Nameh).filter_by(vaseiyat=self.logged_in)
            return dict (log=log,usr=usr,mag=mag,nav=n,payamr=payamr)
  @view_config(route_name='mdelet' )
  def mdelnameh(self):
       id = int(self.request.matchdict['id'])
       page=Session.query(Nameh).filter_by(id=id).one()
       Session.add(Delet_Nameh(nnameh=page.nnameh , mnameh=page.mnameh , chnameh=page.chnameh , manameh=page.manameh,ersal=page.ersal,recive=page.recive,tarikher=page.tarikher,mohlat=page.mohlat,emza=page.emza,jahat=page.jahat,peyvast=page.peyvast,vaseiyat=page.vaseiyat ))
       Session.delete(page)
       HTTPFound('mdelet_nameh')

  @view_config ( route_name='mdelet_nameh' , renderer='template/m_delet_nameh.pt',permission='topsecret' )
  def mnameh_delet(self):
       n=Session.query(Nameh).filter_by(recive=self.logged_in, vaseiyat="F").count()
       page1=Session.query(Delet_Nameh).filter_by(ersal=self.logged_in)
       log=Session.query(User).filter_by(username=self.logged_in)
       payamr=Session.query(payam).filter_by(recive=self.logged_in,state="0").count()
       mag=Session.query(Image).filter_by(name=self.logged_in)
       newre=Session.query(User.username)
       return dict (log=log,mag=mag,page1=page1,nav=n,payamr=payamr,newre=newre)
